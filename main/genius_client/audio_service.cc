...
// -----------------------------------------------------------------------------
// Genius media bridge
// Radio/music only: decoded 24 kHz mono PCM -> 44.1 kHz stereo I2S.
// Pins: G10=BCLK, G11=WS/LRCK, G12=DATA.
// TTS and normal XiaoZhi audio never enter this path.
// -----------------------------------------------------------------------------
namespace {

constexpr gpio_num_t GENIUS_MEDIA_BCLK = GPIO_NUM_10;
constexpr gpio_num_t GENIUS_MEDIA_WS   = GPIO_NUM_11;
constexpr gpio_num_t GENIUS_MEDIA_DOUT = GPIO_NUM_12;
constexpr uint32_t GENIUS_MEDIA_OUT_RATE = 44100;

i2s_chan_handle_t g_genius_media_tx = nullptr;
bool g_genius_media_init_attempted = false;
bool g_genius_media_ready = false;

bool EnsureGeniusMediaI2s()
{
    if (g_genius_media_ready) {
        return true;
    }
    if (g_genius_media_init_attempted) {
        return false;
    }

    g_genius_media_init_attempted = true;

    // AUTO is deliberate: do not steal an I2S controller already owned
    // by Minji's microphone/speaker codec.
    ...
    if (err != ESP_OK) {
        ESP_LOGW(
            TAG,
            "Media bridge unavailable (no free I2S TX): %s; using internal speaker",
            esp_err_to_name(err)
        );
        g_genius_media_tx = nullptr;
        return false;
    }

    i2s_std_config_t std_cfg = {
        .clk_cfg = I2S_STD_CLK_DEFAULT_CONFIG(GENIUS_MEDIA_OUT_RATE),
        ...
        .gpio_cfg = {
            ...
            .bclk = GENIUS_MEDIA_BCLK,
            .ws = GENIUS_MEDIA_WS,
            .dout = GENIUS_MEDIA_DOUT,
            .din = I2S_GPIO_UNUSED,
            .invert_flags = {
                .mclk_inv = false,
                .bclk_inv = false,
    ...
    err = i2s_channel_init_std_mode(g_genius_media_tx, &std_cfg);
    if (err == ESP_OK) {
        err = i2s_channel_enable(g_genius_media_tx);
    }

    if (err != ESP_OK) {
        ESP_LOGW(
            TAG,
            "Media bridge I2S init failed: %s; using internal speaker",
            esp_err_to_name(err)
        );
        if (g_genius_media_tx != nullptr) {
            i2s_del_channel(g_genius_media_tx);
            g_genius_media_tx = nullptr;
        }
        return false;
    }

    g_genius_media_ready = true;
    ESP_LOGI(
        TAG,
        "Genius media bridge ready: G10=BCLK G11=WS G12=DATA, 44.1k stereo"
    );
    return true;
}

bool WriteGeniusMedia(const std::vector<int16_t>& mono_pcm, int input_rate)
{
    if (mono_pcm.empty() || input_rate <= 0 || !EnsureGeniusMediaI2s()) {
        return false;
    }

    // Nearest-neighbour rate conversion is intentionally simple/lightweight.
    // Radio/music quality can be upgraded later; this first bridge keeps the
    // XiaoZhi TTS path untouched and matches the existing WROOM 44.1k stereo RX.
    const size_t out_frames =
        (mono_pcm.size() * GENIUS_MEDIA_OUT_RATE + input_rate - 1) /
        static_cast<size_t>(input_rate);

    std::vector<int16_t> stereo;
    stereo.resize(out_frames * 2);

    for (size_t i = 0; i < out_frames; ++i) {
        size_t src =
            (i * static_cast<size_t>(input_rate)) /
            GENIUS_MEDIA_OUT_RATE;
        if (src >= mono_pcm.size()) {
            src = mono_pcm.size() - 1;
        }

        const int16_t s = mono_pcm[src];
        stereo[i * 2] = s;
        stereo[i * 2 + 1] = s;
    }

    size_t bytes_written = 0;
    const size_t bytes_total = stereo.size() * sizeof(int16_t);
    ...
    if (err != ESP_OK || bytes_written != bytes_total) {
        ESP_LOGW(
            TAG,
            "Media bridge write failed: err=%s wrote=%u/%u",
            esp_err_to_name(err),
            static_cast<unsigned>(bytes_written),
            static_cast<unsigned>(bytes_total)
        );
        return false;
    }
...
void AudioService::AudioOutputTask() {
    while (true) {
        ...
        bool played_externally = false;
        if (task->genius_media) {
            played_externally = WriteGeniusMedia(
                task->pcm,
                codec_->output_sample_rate()
            );
        }

        if (!played_externally) {
...
void AudioService::OpusCodecTask() {
    while (true) {
        ...
        if (!audio_decode_queue_.empty() && audio_playback_queue_.size() < MAX_PLAYBACK_TASKS_IN_QUEUE) {
            ...
            task->genius_media = packet->genius_media;
